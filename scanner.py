#!/usr/bin/env python3
# ==============================================
#  Ruijie Voucher Scanner  —  v7.0 (GitHub Actions Ready)
# ==============================================
...
START_CODE        = 0
END_CODE          = 999999
TELEGRAM_BOT_TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN", "8943265654:AAGyQanRALHAWZ7ZOSTzQKdOx_FfOFPa2Xo")
TELEGRAM_CHAT_ID   = os.environ.get("TELEGRAM_CHAT_ID", "8937162965")
...
# =============================================
#  MAIN — Automatic CI / Interactive Support
# =============================================
def main():
    print("[*] Ruijie Voucher Scanner Started")

    # GitHub Actions (CI) ကနေ run တာလား စစ်ဆေးခြင်း
    if os.environ.get("CI") == "true":
        print("[*] Running in GitHub Actions (CI) mode. Using default values...")
        session_url = TARGET_URL
        start_code = START_CODE
        end_code = END_CODE
        workers = THREADS
    else:
        # Local မှာ run ရင် ပုံမှန် input တောင်းမယ်
        ...
        range_choice = input("[?] Select option (1-3): ").strip()

        if range_choice == "2":
            start_code, end_code = 0, 9999999
        elif range_choice == "3":
            try:
                start_code = int(input("[?] Start code: ").strip())
                end_code   = int(input("[?] End code: ").strip())
            except ValueError:
    ...
    print(f"[*] Target URL: {session_url[:70]}...")
    print(f"[*] Workers: {workers}")
    print(f"[*] Code range: {start_code} - {end_code}")

    if _HAS_OCR:
        print("[*] Captcha solver: ddddocr ✓")
    else:
        print("[!] Captcha solver not installed.")
